              ...
              {Array.isArray(eventos) && eventos.map((evento) =>
                <Space direction="horizontal">
                  <Card
                    ...
                    className="card"
                    actions={[
                      <Button style={{ backgroundColor: "lawngreen", border: "none", color: "darkgreen" }} type="primary" icon={<DownloadOutlined />} onClick={() => { setRedirect(`/lista-participantes/${evento.id}`) }}>Lista de Participantes</Button>
                    ]}
                  >
                    <Meta
                      title={<p style={{ color: "snow" }}>{evento.titulo}</p>}
